...
% note: diagram needed to clarify, add name
If one of the line that pass through it is parallel to y axis. Since we chose the point with the largest $y$ value, the parallel line does not have any intersection above the current point. % thus, the line with the lowest grandient intersect y axis with another line that must intersect the third line closer. This create an contridiction with the assumption that this is the clostest

% Otherwise, the line with the second-largest grandient must intersect.

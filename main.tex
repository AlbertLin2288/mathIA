...
\end{wrapfigure}
Since we will only add one dot, and the new line must go through at least two dots, the new line must pass through one of the pre-existing point. Since the graph is highly symmetric, point D is equivalent to point A, as is point B to C, so we only need to consider one point from each pair, and we only need to consider line with gradient in the range of $[-1,1]$.
